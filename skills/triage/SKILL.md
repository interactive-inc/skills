...
何かしたい時に投げる単一の入口。入力を読み、行き先を決めて記録し、必要なら実装まで進める。書式は `gh-issue-template` と `docs` が正本なのでここには書かない。Issue / PR の本文を組み立てる前に、必ず Skill ツールで `gh-issue-template` を読み込む。
...
引数なしのときは backlog の Propose モードに振る（signals から候補を提案する）。
...
backlog なら `docs` の backlogs 規約に従って記録する。`.docs/index.md` と `.docs/backlogs/index.md` を先に読む。入力が signals/ slug ならその内容を背景としてリンクする。既存 backlogs/ slug を指定された場合は `---` ゾーン分離を解析し、Claude ゾーンのみ書き換える。新規作成時は人間ゾーンを空のまま `---` を置く。`index.md` は触らない。

issue なら Skill ツールで `gh-issue-template` を読み込み、Bug / Feature / Task 判定と Template に従って `gh issue create` する。重複は `gh issue list` で先にチェックする。issue として記録した後、ユーザーが実装に進みたいなら「Workflow: 番号系」に合流する。
...
技術計画を作成し Issue body に書き込む。Skill ツールで `gh-issue-template` を読み込み（未読の場合）、Task Template の書式に従う。計画はコードベースを読んで影響範囲を把握してから作成する。下記「Plan の skip 基準」に該当する場合は計画できない旨を返してユーザーに判断を委ねる。
...
ユーザー入力、認証、データ処理に関わる変更がある場合、`hacker` サブエージェントを起動してセキュリティテストを行う。該当しない変更ではスキップする。

### Verification

変更内容のチェックリストを作成し、実動作を確認する。

ブラウザで動く機能は `playwright-agent` を並列で起動し、別テストファイルを分担させて `bunx playwright test` が通るまで修正させる。
...
`debugger` サブエージェントを起動して変更をレビューする。軽微な問題はその場で修正、重大な指摘は次の Triage に回す。
...
commit-commands:commit-push-pr でプッシュして PR を作成する。PR 本文を書く前に Skill ツールで `gh-issue-template` を読み込み（未読の場合）、PR Template に従う。

## サブエージェント

実装中は必要に応じて責務に合ったサブエージェントに委任する。

Playwright generator は `playwright-agent`。ブラウザ上で動く機能を変更した場合に起動する。並列で別テストファイルを分担させてテストを生成する。
...
`.claude/agents/playwright-*.md` や `@playwright/cli` がプロジェクトに無ければ、実装に入る前に `bootstrap` スキルで Playwright と playwright-agent を導入する。導入後、生成された agent を起動してから進める。
