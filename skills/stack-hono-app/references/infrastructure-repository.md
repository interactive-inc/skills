...
export class CustomerRepository {
  ...
  async findById(id: string): Promise<CustomerEntity | null> {
    const record = await this.db.select().from(customers).where(eq(customers.id, id)).get()
  ...
  async findByEmail(email: string): Promise<CustomerEntity | null> {
    const record = await this.db.select().from(customers).where(eq(customers.email, email)).get()
